...
local function OpenMenu()
    ...
    function RButton2:DoClick()
        if MainFrame.SelectedNode then
            net.Start("RemoveLSScreenResource")
				net.WriteEntity(ent)
				net.WriteString(MainFrame.SelectedNode.res)
			net.SendToServer()
			-- I would do something here to unfuck the menu every time you add and remove something
			-- but DTree is dumb and doesn't work with RemoveItem()
    ...
    function RButton3:DoClick()
        if RText2:GetValue() ~= "" then
            net.Start("AddLSScreenResource")
				net.WriteEntity(ent)
				net.WriteString(RText2:GetValue())
			net.SendToServer()		
        end
    end
...
function ENT:DoNormalDraw(bDontDrawModel)
    ...
    if (EyePos():Distance(self:GetPos()) < rd_overlay_dist and self:GetOOO() == 1) then
        ...
        surface.DrawText("Resource: amount/max amount")
        TempY = TempY + (70 / mul)
        ...
        if (table.Count(self.resources) > 0) then
            ...
            for k, v in pairs(self.resources) do
                ...
                if enttable.network and enttable.network ~= 0 then
                    ...
                    if table.Count(nettable) > 0 then
                        if nettable.resources and nettable.resources[v] then
                            ...
                            if currentnet then
                                othernetworks = currentnet or 0
                            end
                            if currentnet2 then
                                othernetworksres = currentnet2 or 0
                            end
                        else
                ...
                surface.DrawText(tostring(CAF.GetAddon("Resource Distribution").GetProperResourceName(v)) .. ": \t" .. tostring(othernetworksres) .. "/" .. tostring(othernetworks))
                TempY = TempY + (70 / mul)
                i = i + 1
